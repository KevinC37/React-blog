...
function Blogpost() {
  ...
  const localStoragePosts = JSON.parse(window.localStorage.getItem('POSTS_ADDED'));
  const currentPost = _.filter(localStoragePosts, ['id',  Number(window.location.pathname.match(/\d+/)[0])]);
  ...
  const [post, authors ] = useQueries([
    {queryKey: 'post', queryFn: fetchPost, enabled: (typeof dedupedPost == 'undefined')},
    {queryKey: 'author', queryFn: fetchAuthor, enabled: (typeof dedupedPost == 'undefined')},
  ]);
